<template>
	<form @submit.prevent="selectPage" class="form-inline">
		<select v-model="updatedPage" class="form-control">
			<option value="" disabled>Select Page</option>
			<option v-for="page in pages" :value="page">{{ page }}</option>
		</select>
		<input class="btn btn-info change-page form-control" type="submit" value="Change Page">
	</form>
</template>
